...
function performInitialRepositoryAnalysis(user, githubRepository, progressCallback, mainCallback) {
    ...
    ], function(error, githubRepositoryReport) {
        if (error) throw error;
...
function performRepositoryAnalysis(user, githubRepository, githubRepositoryReport, progressCallback, mainCallback) {
    ...
    async.each(githubRepositoryReport.tree, function(file, asyncCallback) {
        analyseFile(user, githubRepository.name, githubRepositoryReport.sha, file.path, function(report) {
            ...
            file.numericGrade = grade.numericGrade(report);
            file.grade = grade.letterGradeFromNumeric(file.numericGrade);
        ...
        // Calculate aggregate grade
        var numericGrade = 0;
        githubRepositoryReport.tree.forEach(function(file) {
            numericGrade += file.numericGrade;
        });
        numericGrade = githubRepositoryReport.tree.length / numericGrade;
        githubRepositoryReport.numericGrade = numericGrade;
        githubRepositoryReport.grade = grade.letterGradeFromNumeric(numericGrade);
